...
const lettersHtmlMobile = elementMobile.textContent.replace(/\S/g, '<span class="letter">$&</span>');
...
tlMobile.add({
    ...
    translateX: lettersMobile.map((letter, i) =>
        ({ value: letter.offsetLeft + letter.offsetWidth, duration: 1, delay: i === 0 ? 0 : JUMP_AFTER_MSMobile }))
}, TYPE_AFTER_MSMobile)
tlMobile.add({
...
const lettersHtmlEmial = elementEmial.textContent.replace(/\S/g, '<span class="letter">$&</span>');
...
tlEmial.add({
    ...
    translateX: lettersEmial.map((letter, i) =>
        ({ value: letter.offsetLeft + letter.offsetWidth, duration: 1, delay: i === 0 ? 0 : JUMP_AFTER_MSEmial }))
}, TYPE_AFTER_MSEmial)
tlEmial.add({
